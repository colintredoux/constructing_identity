...
```{r loadlibs}
library(needs)
needs(tidyverse, psych, lavaan, lavaan.survey, haven, magrittr, nlme,
      stringr, janitor, slipper)
...
id_all.dat <- full_join(id_equal.dat,id_unequal.dat) %>% 
   ...
   rename(ingroup_favoritism = igf,
          outgroup_aid = ogg) %>% 
   mutate(equality_cond    = ifelse(str_sub(gamename,1,1) == "E","equality",
                                    "inequality")) %>% 
   mutate(indiv_group_cond =  case_when(
              str_sub(gamename, 2,2) == "I" ~ "individual",
              str_sub(gamename, 2,2) == "M" ~ "wealthy_minority",
              str_sub(gamename, 2,2) ==  1 ~ "group",
              str_sub(gamename, 2,2) ==  2 ~ "group",
              TRUE ~ "not found"
              )
   )
...
id_all.dat %>% 
    ...
    summarise(sd = sd(deltatokens, na.rm = T)/10)

id_all.dat %>% 
    select(group_condition, deltatokens, uid) %>% 
    spread(key = group_condition, value = deltatokens) %>% 
    select(-uid) -> x
...
fligner.test(deltatokens ~ group_condition, data=id_all.dat)

(data=id_all.dat %>% filter(group_condition == "group_equality" | 
                            group_condition == "group_inequality") %$%
                            fligner.test(deltatokens ~ group_condition))
(data=id_all.dat %>% filter(group_condition == "group_equality" | 
...
id_all.dat %>% 
    select(uid, id, pid, group_condition, gameno) %>% 
    gather(key = time, value = socid, id:pid) %>% 
